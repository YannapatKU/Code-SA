...
MainNavbarDom.innerHTML = htmlMainNavData;
    ...
    const CrimeDom = document.getElementById('CrimeData');
    ...
    let CrimeBooks = response.data.filter(book => book.BookType === 'นิยาย' && book.SubType === 'Crime');
    for (let i = 0; i < 12 && i < CrimeBooks.length; i++) {
        let book = CrimeBooks[i];    
        htmlContent += `
    ...
    htmlContent += '</div></div>'; 
    CrimeDom.innerHTML = htmlContent;
