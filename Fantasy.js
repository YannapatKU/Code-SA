...
MainNavbarDom.innerHTML = htmlMainNavData;
    ...
    const FantasyDom = document.getElementById('FantasyData');
    ...
    let FantasyBooks = response.data.filter(book => book.BookType === 'นิยาย' && book.SubType === 'Fantasy');
    for (let i = 0; i < 12 && i < FantasyBooks.length; i++) {
        let book = FantasyBooks[i];    
        htmlContent += `
    ...
    htmlContent += '</div></div>'; 
    FantasyDom.innerHTML = htmlContent;
